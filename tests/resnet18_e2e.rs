...
fn resnet18_predictions_differ_for_different_inputs() {
    ...
    let outs1 = model.run(&[&input1]).expect("inference failed");
    let logits1 = outs1[0].as_slice::<f32>().to_vec();

    // Input 2: ones
    let input2 = Buffer::from_slice::<f32>(&vec![1.0f32; size], &[1, 3, 224, 224], DType::F32);
    let outs2 = model.run(&[&input2]).expect("inference failed");
    let logits2 = outs2[0].as_slice::<f32>().to_vec();
